...
COSMOS_DB_ENDPOINT = "https://cdb-adr5-env15.documents.azure.com:443/"
DATABASE_NAME = "state"
...
def fetch_latest_prompt_by_type(issue_type: str) -> str:
    """
    ...
    try:
        ...
        if prompt_item:
            logging.info(f"Found prompt for type '{issue_type}'")
            return prompt_item.get('guideline_prompt', '')
        else:
            logging.error(f"No prompt found for type '{issue_type}'")
            return ''
    except Exception as e:
        logging.error(f"Error fetching prompt for type '{issue_type}': {e}")
        return ''
