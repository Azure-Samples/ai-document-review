...
export function IssueCard({ docId, issue, selected, onSelect, onUpdate }: IssueCardProps) {
  ...
  return (
    <Card
      ...
      selected={selected}
      onSelectionChange={() => onSelect(issue)}
    ...
    >
      <CardHeader
        ...
        header={
          <Caption1Strong strikethrough={issue.status === IssueStatus.Dismissed}>
            {issue.text}
          </Caption1Strong>
        }
      />
      ...
      {selected && (
        <>
          <Field label="Explanation" className={classes.explanation}>
            <Textarea
              defaultValue={
                issue.modified_fields?.explanation
                  ? issue.modified_fields.explanation
                  : issue.explanation
              }
          ...
          <Field label="Suggested fix">
            <Textarea
              defaultValue={
                issue.modified_fields?.suggested_fix
                  ? issue.modified_fields?.suggested_fix
                  : issue.suggested_fix
              }
              readOnly={issue.status !== IssueStatus.NotReviewed}
          ...
          {addFeedback && (
            <Card appearance="outline" className={classes.feedback}>
              <CardHeader
                image={
                  <PersonFeedback20Filled primaryFill={tokens.colorPaletteDarkOrangeForeground1} />
                }
