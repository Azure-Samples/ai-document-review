...
function Review() {
  ...
  const [agentConfig, setAgentConfig] = useState<
    Record<string, { color: 'informative' | 'danger' | 'brand' | 'important'; description: string }>
  >({})
  ...
  const getAgentConfig = useCallback(() => {
    ...
    issues.forEach((issue) => {
      if (!agentConfig[issue.type]) {
        agentConfig[issue.type] = {
          color: typeIndex % 2 === 0 ? 'informative' : 'danger',
          description: ''
        }
        typeIndex++
      }
          ...
          switch (msg.event) {
            case APIEvent.Issues: {
              ...
              for (const i of newIssues) {
                ...
                if (
                  i.location &&
                  i.location.page_num !== undefined &&
                  i.location.bounding_box &&
                  i.location.bounding_box.length > 0
                ) {
                  try {
                    ;[pdfBytesWithAnnotations] = addAnnotation(
                      i.location.page_num,
                      i.location.bounding_box
                    )
                  } catch (e) {
  ...
  return (
    <div className={classes.content}>
        ...
        {docId && (
          <div className={classes.issues}>
            {issues
              ...
              .filter(
                (issue) =>
                  statusFilter.includes(issue.status) && !hideTypesFilter.includes(issue.type)
              )
            ...
            {checkInProgress && (
              <Skeleton>
                {Array.from({ length: 7 }, (_, i) => (
                  <SkeletonItem key={i} className={classes.skeletonItem} />
                ))}
              </Skeleton>
            )}
          </div>
      ...
      <Card className={classes.sidebar}>
        {checkError && <ErrorMessage title="Error running check" message={checkError} />}
        {docId && (
          <Button
        ...
        <Field aria-label="Filter by status" size="small">
          <TagPicker onOptionSelect={onStatusFilterSelect} selectedOptions={statusFilter}>
            <TagPickerControl>
            ...
            <CardHeader
              ...
              action={
                <Button
                  ...
                  icon={hideTypesFilter.includes(entry[0]) ? <EyeOff20Regular /> : <Eye20Regular />}
